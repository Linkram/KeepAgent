...
**Settings → App updates → Check for updates** also offers
...
In **Settings → Other phone apps**, choose **Ask each action** or explicitly allow
...
- **Connections (F-010 surface)**: named OpenAI-compatible API endpoints —
  ...
  the chat area.
- **LLM provider**: `core/llm` — an OpenAI-compatible SSE client
...
- **Streaming chat (F-003/F-004)**: token-by-token text, a collapsible "Thought"
...
- **Agent loop + approval gate (F-011)**: `core/agent` runs up to 12
  ...
  toggle.
- **File tools**: `core/fs` (`read`, `write`, `edit`, `glob`,
...
- **The web test target is partial** — local WebView loading and screenshot
  attachment work, while console/DOM capture and background self-testing
  remain for F-006. Desktop profile and native Android targets are still
  planned (F-013/F-015); the on-device toolchain (F-016) and remote desktop
  runner (M3, F-018) are not built.
